...
- Source URL: https://www.python-httpx.org/advanced/clients
- Page Title: Clients - HTTPX
- Semantic Filename: clients_httpx.md
- Ollama Model: llama3.2:3b
- Export Mode: Smart Deep Crawl
- Statistics:
  - Status: Success
  - Content Length: 11,093 characters
---
...
On the other hand, a `Client` instance uses `Client` will reuse the underlying TCP connection, instead of recreating one for every single request.
...
Or an alternate example, this time using the 
```
importtempfile
importhttpx
importrich.progress
...
More specifically, if a tuple is used as a value, it must have between 2 and 3 elements:
  ...
  * An optional third element can be used to specify the `None` then HTTPX will not include a content-type MIME header field.
