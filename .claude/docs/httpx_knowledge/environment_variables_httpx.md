...
- Source URL: https://www.python-httpx.org/environment_variables
- Page Title: Environment Variables - HTTPX
- Semantic Filename: environment_variables_httpx.md
...
```
exportHTTP_PROXY=http://my-external-proxy.com:1234

# This request will be sent through the proxy
python"import httpx; httpx.get('http://example.com')"

# This request will be sent directly, as we set `trust_env=False`
python"import httpx; httpx.get('http://example.com', trust_env=False)"
...
```
exportHTTP_PROXY=http://my-external-proxy.com:1234
exportNO_PROXY=http://127.0.0.1,python-httpx.org
...
python"import httpx; httpx.get('http://127.0.0.1:5000/my-api')"
python"import httpx; httpx.get('https://www.python-httpx.org')"
...
## `SSL_CERT_DIR`
Valid values: a directory following an 
If this environment variable is set and the directory follows an `c_rehash`) then HTTPX will load CA certificates from this directory instead of the default location.
Example:
```
